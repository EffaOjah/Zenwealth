...
// Function to update the user's has_completed_yt_reward column
...
async function updateHasWithdrawnColumn(status, userId) {
    return new Promise((resolve, reject) => {
        connection.query('UPDATE users SET has_withdrawn = ? WHERE user_id = ?', [status, userId], (err, result)=>{
            if (err) {
                console.log(err);
                reject(err);
            } else{
                console.log(`Successfully updated the has_withdrawn column of the user to ${status}`);
                resolve(result);
            }
        });
    });
    ...
    return new Promise((resolve, reject) => {
        connection.query('SELECT * FROM pop_up_ad', (err, result)=>{
            if (err) {
                ...
                console.log('Pop up content: ', result);
                resolve(result);
            }
        });
    });
