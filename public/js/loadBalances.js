...
document.addEventListener('DOMContentLoaded', async ()=>{
    ...
    let directReferralValue = data.getTotalDirectReferralBalance[0].balance / 1500;
    let indirectReferralValue = data.getTotalIndirectReferralBalance[0].balance / 1500;

    let sum = directReferralValue + indirectReferralValue;

       totalAffiliateBalanceSpan[0].innerHTML = `${Math.floor(sum * 100) / 100}`;
    ...
    let totalAffiliateBalanceValue = data.getTotalAffiliateBalanceView[0].affiliateBalance / 1500;
       ...
       availabletotalAffiliateBalanceSpan[0].innerHTML = `${Math.floor(totalAffiliateBalanceValue * 100) / 100}`;
     ...
     let totalZenpointsValue = data.getTotalZenPointsView[0].ZenPoints / 1500;
       ...
       ZenPointsSpan[1].innerHTML = !data.getTotalZenPointsView[0].ZenPoints ? `$0` : `$${Math.floor(totalZenpointsValue * 100) / 100}`;
     ...
     let getTotalDirectReferralValue = data.getTotalDirectReferralBalance[0].balance / 1500;
       ...
       directReferralBalance[1].innerHTML = `$${Math.floor(getTotalDirectReferralValue * 100) / 100}`;
     ...
     let getTotalIndirectReferralValue = data.getTotalIndirectReferralBalance[0].balance / 1500;
       ...
       indirectReferralBalance[1].innerHTML = `$${Math.floor(getTotalIndirectReferralValue * 100) / 100}`;
